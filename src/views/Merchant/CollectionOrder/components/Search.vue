              ...
              return (
                <div>
                  <ActionButton
                    ...
                    showNotProcessing={propsComputed.showNotProcessing}
...
const exportData = async () => {
  ...
  await elFormExpose?.validate(async (isValid) => {
    if (isValid) {
      const model = await filterModel()
      emit('exportData', model)
    }
  })
}

const notProcessing = async () => {
  const elFormExpose = await getElFormExpose()
  await elFormExpose?.validate(async (isValid) => {
    if (isValid) {
      const model = await filterModel()
      emit('notProcessing', model)
    }
  })
}
const total = async () => {
  ...
  <template v-if="layout === 'bottom'">
    <div :style="bottomButtonStyle">
      <ActionButton
        ...
        :show-not-processing="getProps.showNotProcessing"
